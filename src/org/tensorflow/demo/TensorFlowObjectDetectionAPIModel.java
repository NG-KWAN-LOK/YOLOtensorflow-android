...
public class TensorFlowObjectDetectionAPIModel implements Classifier {
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap) {
    ...
    for (int i = 0; i < outputScores.length; ++i) {
      ...
      final PointF point =
              new PointF(
                      (outputLocations[i + 1] + outputLocations[i + 3])/2 * inputSize,
                      (outputLocations[i] + outputLocations[4 * i + 2])/2 * inputSize);
      pq.add(
          new Recognition("" + i, labels.get((int) outputClasses[i]), outputScores[i], detection));
      if(outputScores[i] > 0.1) {
        Log.v(TAG, "i = " + outputLocations[i] * inputSize + "class=" + labels.get((int) outputClasses[i]) + " scores = " + outputScores[i] + " detection = " + point);
      }
    }
